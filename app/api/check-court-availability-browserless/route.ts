...
export async function GET(request: Request) {
  ...
  const extractionResult = await page.evaluate(() => {
    ...
    // Time slots appear on the right side of the calendar after date selection
    // Look for time slot elements - they're usually buttons on the right side
    const seenText = new Set();
    
    // First, try to find time slot containers (they're usually on the right side)
    // Look for common time slot container patterns
    const timeSlotContainers = document.querySelectorAll('[class*="time"], [class*="slot"], [class*="available"], [class*="hour"], [id*="time"], [id*="slot"]');
    
    // Also check all buttons and clickable elements for time patterns
    const clickableElements = document.querySelectorAll('button, [role="button"], a, [class*="click"], [onclick]');
    
    // Combine both approaches - check containers first, then all clickable elements
    const elementsToCheck = Array.from(timeSlotContainers).concat(Array.from(clickableElements));
    ...
    elementsToCheck.forEach((el) => {
      ...
      if (['next', 'back', 'skip', 'book now', 'more details'].includes(text.toLowerCase())) {
        return;
      }
      ...
      if (isTimePattern) {
        ...
        const isDisabled = el.hasAttribute('disabled') || 
                          ...
                          el.classList.contains('Day'); // Skip calendar day elements
    ...
    // Also try specific selectors as additional search
    const selectors = [
      'button[class*="time"]:not([disabled]):not([class*="Day"])',
      '[class*="time-slot"] button:not([disabled])',
      '[class*="slot"] button:not([disabled]):not([class*="date"]):not([class*="Day"])',
      '[class*="available"] button:not([disabled])',
      '[role="button"][class*="time"]:not([disabled])',
      'button:not([disabled]):not([class*="Day"]):not([class*="disabled"])'
    ];
